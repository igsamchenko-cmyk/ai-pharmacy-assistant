...
function assertLocalDatabase(): void {
  ...
  if (!value)
    throw new Error("National-list DB smoke requires a test database.");
...
async function main(): Promise<void> {
  ...
  const snapshot = JSON.parse(
    readFileSync(
      resolveDataFilePath("data/national-list/ua-2025-10-10.json", {
  ...
  try {
    ...
    if (
      first.persistedEntries !== snapshot.counts.valid ||
      second.insertedEntries !== 0
    ) {
    ...
    if (
      reviewed.rows[0]?.status !== "reviewed" ||
      Number(reviewed.rows[0]?.entries ?? 0) !== snapshot.counts.valid
    ) {
      throw new Error(
        "Committed national-list release is not a reviewed immutable snapshot.",
      );
    }
    ...
    const performanceRegressionPct =
      baselineWarmMs > 0 ? (performanceRegressionMs / baselineWarmMs) * 100 : 0;
    ...
    if (
      activated.products !== Number(before.rows[0]?.products ?? 0) ||
    ...
    if (
      !/national_list_match_release_product_idx|national_list_match_status_idx/iu.test(
        planText,
      )
    ) {
      throw new Error(
        "National-list resolver query plan did not use a bounded match index.",
      );
    }
    ...
    if (
      Number(after.rows[0]?.products ?? 0) !==
        Number(before.rows[0]?.products ?? 0) ||
      Number(after.rows[0]?.mappings ?? 0) !==
        Number(before.rows[0]?.mappings ?? 0)
    ) {
      throw new Error(
        "National-list lifecycle changed existing registry or mapping rows.",
      );
    }
    console.log(
      JSON.stringify(
...
main().catch((error: unknown) => {
  const message =
    error instanceof Error ? error.message : "National-list DB smoke failed.";
  console.error(
    message.replace(/(?:postgres(?:ql)?):\/\/\S+/giu, "[redacted]"),
  );
  process.exitCode = 1;
});
