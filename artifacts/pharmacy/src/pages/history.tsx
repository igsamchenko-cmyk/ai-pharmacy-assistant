      ...
      <div className="space-y-3">
        {isLoading ? (
          ...
          <div className="text-center py-12 px-4 text-destructive border-2 border-dashed border-destructive/30 rounded-xl">
            <p>Не вдалося завантажити історію. Спробуйте оновити сторінку.</p>
          </div>
        ) : !history || history.length === 0 ? (
          <div className="text-center py-16 px-4 border-2 border-dashed border-border rounded-xl">
            ...
            <p className="text-sm text-muted-foreground mt-2">
              Тут будуть зберігатися ваші останні пошуки та перевірки.
            </p>
          </div>
        ) : (
          history.map((entry) => {
            ...
            return (
              <Card
                key={entry.id}
                className="overflow-hidden group hover:border-border/80 transition-colors"
              >
                <CardContent className="p-4 flex items-start justify-between gap-4">
                  <div className="flex gap-4 min-w-0 flex-1">
                    <div
                      className={`p-2 rounded-lg shrink-0 flex items-center justify-center ${config.color.split(" ")[0]}`}
                    >
                    ...
                    <div className="min-w-0 flex-1 space-y-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        <Badge
                          variant="outline"
                          className={`text-[10px] uppercase font-bold ${config.color}`}
                        >
                          {config.label}
                        </Badge>
                  ...
                  <button
                    ...
                    title="Видалити"
                    aria-label={`Видалити запис: ${entry.title}`}
                  >
