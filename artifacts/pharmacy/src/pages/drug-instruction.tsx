...
const QUICK_SECTION_KEYS: SectionKey[] = [
  "indications",
  "contraindications",
  "specialWarnings",
  "interactions",
...
export function InstructionSectionContent({ content }: { content: string | null }) {
  return content ? (
    <p className="whitespace-pre-wrap break-words">{content}</p>
  ) : (
    <p className="text-muted-foreground">
      Розділ ще не структуровано. Відкрийте офіційну інструкцію нижче.
    </p>
  );
}
...
function InstructionLoading() {
  return (
    <div className="mx-auto max-w-4xl space-y-6" aria-label="Завантаження інструкції">
  ...
  return (
    <main className="mx-auto w-full max-w-4xl overflow-x-hidden pb-16">
      ...
      <header className="space-y-4 border-b pb-6">
        ...
        <div>
          <h1 className="break-words text-2xl font-bold sm:text-3xl">{data.tradeName}</h1>
        ...
        <dl className="grid gap-3 text-sm sm:grid-cols-2">
          <div>
            <dt className="text-xs text-muted-foreground">Форма та дозування</dt>
            <dd className="mt-1 break-words">{data.dosageForm}, {data.strength}</dd>
          </div>
          <div>
            <dt className="text-xs text-muted-foreground">Виробник</dt>
            <dd className="mt-1 break-words">{data.manufacturer}, {data.manufacturerCountry}</dd>
          </div>
          <div>
            <dt className="text-xs text-muted-foreground">Реєстраційне посвідчення</dt>
            <dd className="mt-1 break-words font-mono">{data.registrationNumber}</dd>
          </div>
          <div>
            <dt className="text-xs text-muted-foreground">Дата офіційного документа</dt>
      ...
      <section className="py-6" aria-labelledby="instruction-contents-title">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h2 id="instruction-contents-title" className="text-lg font-semibold">
              Зміст інструкції
            </h2>
        ...
        <nav className="mt-4 flex flex-wrap gap-x-4 gap-y-2 text-sm" aria-label="Розділи інструкції">
          {INSTRUCTION_SECTION_LABELS.map(({ key, label }) => (
            <a key={key} href={`#instruction-${key}`} className="text-primary hover:underline">
              {label}
            </a>
          ))}
        </nav>
          ...
          {visibleSections.map(({ key, label }) => {
            ...
            return (
              <details
                ...
                open={key === "indications" && !sectionSearch ? true : undefined}
              >
      ...
      <footer className="border-t pt-6">
        <p className="text-sm text-muted-foreground">
          Джерело: Державний реєстр лікарських засобів України. Ліцензія набору даних: {data.source.license}.
        </p>
