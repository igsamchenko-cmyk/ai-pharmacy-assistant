...
import { useScanPackage, useCreateHistory, getListHistoryQueryKey } from "@workspace/api-client-react";
...
import { Scan as ScanIcon, UploadCloud, Search, Pill, ChevronRight, Info, Image as ImageIcon } from "lucide-react";
  ...
  return (
    <div className="space-y-6 animate-in fade-in pb-10">
      <div className="space-y-1">
        ...
        <p className="text-sm text-muted-foreground">Завантажте фото упаковки або рецепта для швидкого пошуку.</p>
      </div>
          ...
          >
            ...
            {preview ? (
              ...
              <div className="text-center space-y-4 py-6">
                ...
                <div>
                  <p className="font-bold text-foreground">Натисніть для завантаження фото</p>
                  <p className="text-sm text-muted-foreground mt-1">Підтримуються формати JPG, PNG</p>
                </div>
              </div>
            )}
          </div>
      ...
      <div className="space-y-3">
        <p className="text-sm font-bold text-foreground">Або введіть текст вручну:</p>
        <Textarea 
        ...
        <div className="space-y-6 pt-6 animate-in slide-in-from-bottom-4 fade-in">
          {!scanPackage.data.ocrAvailable && preview && (
            <Alert className="bg-accent/30 border-accent">
              ...
              <AlertDescription>Штучний інтелект для розпізнавання тексту не налаштовано. Будь ласка, використовуйте ручне введення тексту.</AlertDescription>
            </Alert>
          )}
          ...
          <div className="space-y-4">
            <h2 className="text-xl font-bold flex items-center justify-between">
              ...
              <Badge variant="outline" className="text-xs bg-muted">{scanPackage.data.matches.length} збігів</Badge>
            </h2>

            {scanPackage.data.text && (
              <div className="p-3 bg-muted/40 rounded-lg text-sm text-muted-foreground border border-border">
                <span className="font-bold text-foreground mb-1 block">Розпізнаний текст:</span>
                "{scanPackage.data.text}"
              </div>
            )}
            ...
            {scanPackage.data.matches.length === 0 ? (
              <div className="text-center py-10 px-4 border-2 border-dashed border-border rounded-xl">
                ...
                <p className="text-muted-foreground font-medium">Не знайдено жодного препарату за цими даними.</p>
                <Button variant="link" className="mt-2 text-primary" onClick={() => scanPackage.reset()}>Спробувати ще раз</Button>
              </div>
            ) : (
              <div className="space-y-3">
                      ...
                      <CardContent className="p-4 flex items-center justify-between gap-4">
                        <div className="flex gap-3 items-center flex-1 min-w-0">
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <p className="text-xs text-muted-foreground truncate">{drug.inn} • {drug.form}</p>
                          </div>
                        </div>
