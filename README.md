...
лікарських засобів, точні реєстрові профілі, регуляторні оновлення, перевірка
взаємодій і контекстні професійні інструменти. Інтерфейс українською, mobile-first.
...
> приймає лікар. Професійний профіль окремо показує стан, дату й повноту кожного
> офіційного джерела; локальні демонстраційні дані не видаються за підтверджені
> реєстрові факти. Деталі — у
...
- **Професійний профіль** — точна реєстрова позиція, чинність реєстрації,
  Rx/OTC, Нацперелік, «Доступні ліки», гранична ціна та походження кожного
  статусу.
- **Заборони та оновлення** — версійований знімок розпоряджень Держлікслужби з
  автоматичною перевіркою актуальності при відкритті застосунку.
...
- **Фармаконагляд** — клінічна чернетка для точної реєстрової позиції без
  збереження персональних даних, з переходом до офіційної АІСФ ДЕЦ.
- **Історія** — довідкові запити зберігаються в базі даних; записи можна
  видаляти поодинці або очистити всі.

## Локальні метрики пошуку

Прихована сторінка `/perf` показує TTIR, TTFR і TTC для cold/warm запусків.
Останні 200 сесій зберігаються лише в IndexedDB поточного браузера й не
надсилаються на сервер. Сторінка завантажується окремим lazy-чанком.

## Нормалізація пошуку

Локальний пошук зберігає чинне ранжування торгової назви, реєстраційного
номера, МНН, транслітерації, форми й дозування. Окремий Web Worker додає
виправлення QWERTY ↔ ЙЦУКЕН і контрольований SymSpell fuzzy на відстані 1.
Точні/layout-збіги повертаються як `primary`; fuzzy — лише як окремі
`suggested`, тільки коли `primary` порожній. Виправлені результати ніколи не
відкривають картку автоматично. SymSpell-мапа будується ліниво при першому
fuzzy-запиті, тому cold-побудова основного індексу не дорожчає.
## PWA та швидкий старт пошуку

FarmAssist встановлюється як PWA на Android та iOS. Service worker
реєструється лише після window.load і кешує оболонку пошуку та картку
препарату; операторські lazy-розділи й /perf до precache не входять.
Запити /api/catalog/** завжди використовують NetworkOnly: джерелом
офлайн-пошуку лишається версійований каталог у IndexedDB, який тихо перевіряє
оновлення через 30 секунд після warm-старту.

На екрані пошуку поле є першим робочим елементом. На desktop воно отримує
фокус автоматично й приймає друк поза іншими полями; на смартфоні клавіатура
відкривається лише після тапу. Під полем локально зберігаються п’ять останніх
успішних запитів. Основна ідентичність останніх відкритих реєстрових позицій
кешується окремо, тому офлайн-картка показує торгову назву, МНН, форму,
дозування та реєстраційний номер без удаваної доступності актуальних статусів.
